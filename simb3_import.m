%% Dataset import (CRREL IMB 2005-2015)
close all; clear; clc; 
ncvars =  {'time','lat','lon','z','hi','hs','sur','int','bot','T','hi_west','hs_west','sur_west','int_west','bot_west'};
projectdir = 'C:\Users\Evgenii\Documents\MATLAB\Cryosphere Innovation';
...
%%
figure
tile = tiledlayout(3,2); tile.TileSpacing = 'compact'; tile.Padding = 'none';
nexttile([1 2])
load('batlow50.mat'); 
for i = 1:length(t)
...
%% Dataset import (CRREL IMBs 2005-2015, 2019-2024)
close all; clear; clc; 
ncvars =  {'time','lat','lon','z','hi','hs','sur','int','bot','T'};
projectdir = 'C:\Users\Evgenii\Documents\MATLAB\Cryosphere Innovation';
...
hYLabel = ylabel('Depth (m)'); set([hYLabel gca],'FontSize',7,'FontWeight','normal');
hBar1 = colorbar; ylabel(hBar1,'Vertical temp. grad. (°C/m)','FontSize',7);
name=convertStringsToChars(label{i}); title(sprintf('Vert. temp. gradient, %s',name),'FontSize',8,'FontWeight','normal');
leg = legend('','Updated','box','on','NumColumns',1); set(leg,'FontSize',6,'Location','northwest','orientation','horizontal'); leg.ItemTokenSize = [30*0.5,18*0.5];
ylim([round(min(int{i}),1)-0.2 round(max(sur{i}),1)+0.1]);
datetick('x','mmm','keepticks','keeplimits'); xtickangle(0);